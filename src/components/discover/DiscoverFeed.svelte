...
<div class="mx-auto max-w-3xl px-4 pb-6">
  <div
    ...
    class={`sticky top-24 z-20 bg-white transition-opacity duration-300 ease-in-out motion-reduce:transition-none md:top-14 ${showSticky ? 'opacity-100' : 'pointer-events-none opacity-0'}`}
  >
        ...
        <div class="flex flex-wrap gap-2">
          {#each FILTER_OPTIONS.filter((option) => ['latest', 'upcoming'].includes(option.value)) as option (option.value)}
            <button
              ...
              class={`rounded-full px-3 py-1.5 text-sm font-medium whitespace-nowrap transition-colors ${filter === option.value ? 'bg-green-600 text-white' : 'bg-zinc-100 text-zinc-600 hover:bg-zinc-200'}`}
            >
              {option.label}
            </button>
          {/each}
